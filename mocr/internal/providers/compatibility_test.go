...
func TestUnsupportedTemperatureRetriesAndCaches(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if _, ok := body["temperature"]; ok {
			w.WriteHeader(400)
			w.Write([]byte(`{"error":{"message":"Unsupported parameter: 'temperature' is not supported with this model."}}`))
			return
		}
	...
	for i := 0; i < 2; i++ {
		text := ""
		_, err := Generate(context.Background(), GenerateOptions{BaseURL: server.URL, APIKey: "mock", ModelID: "custom-reasoner", Stream: true}, func(chunk string) bool { text += chunk; return true })
		if err != nil || text != "ok" {
...
func TestEssentialParametersNotSilentlyRemoved(t *testing.T) {
	for _, field := range []string{"tools", "messages", "reasoning_effort", "max_tokens"} {
		...
		if optionalRejectedField(raw) != "" {
			t.Fatalf("essential field %s removed", field)
		}
	}
}
