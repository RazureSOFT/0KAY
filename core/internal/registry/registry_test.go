...
func TestDependenciesGateDiscovery(t *testing.T){
 r:=NewRegistry();agent,_:=r.Register(&pluginv1.PluginInfo{Name:"agent"},[]string{"agent","requires:mocr"},"localhost:50054")
 if len(r.GetAgents(true))!=0||len(r.MissingDependencies(agent))!=1 {t.Fatal("missing dependency did not block scheduling")}
 r.Register(&pluginv1.PluginInfo{Name:"mocr"},[]string{"mocr"},"localhost:50052")
 if len(r.GetAgents(true))!=1 {t.Fatal("dependency restoration did not enable executor")}
 r.SetEnabled("mocr",false);if len(r.GetAgents(true))!=0 {t.Fatal("disabled dependency was ignored")}
}
