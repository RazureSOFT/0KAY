...
func (g *Gateway) handleAgentApprovals(w http.ResponseWriter,r *http.Request) {
 ...
 var body struct{ExecutorID string `json:"executor_id"`;ID string `json:"id"`;Allow *bool `json:"allow"`;Answer *string `json:"answer"`}
 if r.Method=="POST" && (json.NewDecoder(http.MaxBytesReader(w,r.Body,65536)).Decode(&body)!=nil || (!questions&&body.Allow==nil)||(questions&&body.Answer==nil)) {http.Error(w,"invalid decision",400);return}
 ...
 for _,agent:=range g.registry.GetAgents(true) {
  ...
  if questions {tool="question_list"}
  if r.Method=="POST" {if questions {tool="question_answer";args=map[string]interface{}{"id":body.ID,"answer":*body.Answer}}else{tool="approval_decide";args=map[string]interface{}{"id":body.ID,"allow":*body.Allow}}}
