...
func (g *Gateway) handleUsageRecord(w http.ResponseWriter, r *http.Request) {
	...
	if json.NewDecoder(http.MaxBytesReader(w, r.Body, 16384)).Decode(&record) != nil || record.RequestID == "" || record.PromptTokens < 0 || record.CompletionTokens < 0 {
		http.Error(w, "invalid usage", 400)
		return
	}
