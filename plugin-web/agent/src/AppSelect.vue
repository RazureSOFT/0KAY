...
function layout(){
 ...
 upwards.value=below<Math.min(280,visible.value.length*46+58)&&above>below
...
<style>
/* Global select — Material 3 Expressive. Menu is teleported to <body>, so its
   rules stay unwrapped while the trigger is scoped under #app. */
...
.app-select-option.selected.highlighted{background:color-mix(in srgb,var(--md-primary-container) 88%,var(--md-primary) 12%)}
