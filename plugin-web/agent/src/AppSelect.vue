...
const props = withDefaults(defineProps<{ modelValue?: string; options: Array<string | Option>; disabled?: boolean; placeholder?: string; ariaLabel?: string; searchable?: boolean }>(), { modelValue:'', placeholder:'请选择', disabled:false, searchable:false })
...
function layout(){
 ...
 upwards.value=below<Math.min(280,visible.value.length*40+52)&&above>below
...
async function keydown(event:KeyboardEvent){
 ...
 if(['ArrowDown','ArrowUp','Home','End','Enter',' '].includes(event.key)){
  if(props.searchable&&event.key===' ')return
...
<template>
 <div v-bind="$attrs" class="app-select" :class="{ 'is-disabled':disabled }">
  ...
  <Teleport to="body"><Transition name="select-menu"><div v-if="opened" :id="id" ref="menu" class="app-select-menu" :class="{'opens-up':upwards}" :style="position" role="listbox" :aria-label="ariaLabel || '选项'" @pointerdown.prevent>
   <label v-if="searchable" class="app-select-search"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="1.8"/><path d="m20 20-3.5-3.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg><input ref="searchInput" v-model="searchQuery" type="text" :placeholder="locale==='en'?'Search…':'搜索…'" @keydown="keydown" /></label>
   ...
   <div v-if="!visible.length" class="app-select-empty">{{ locale==='en'?'No matches':'没有匹配项' }}</div>
